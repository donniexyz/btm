...
public class BitronixTransactionManager implements TransactionManager, UserTransaction, Referenceable, Service {
    ...
    public BitronixTransactionManager() {
        try {
            ...
            Duration backgroundRecoveryInterval = TransactionManagerServices.getConfiguration().getBackgroundRecoveryInterval();
            if (backgroundRecoveryInterval.isNegative() || backgroundRecoveryInterval.isZero()) {
                throw new InitializationException("invalid configuration value for backgroundRecoveryInterval, found '" + backgroundRecoveryInterval + "' but it must be greater than 0");
            }

            inFlightTransactions = createInFlightTransactionsMap();

            if (log.isDebugEnabled()) {
                log.debug("recovery will run in the background every {} second(s)", backgroundRecoveryInterval);
            }
            LocalDateTime nextExecutionDate = Instant.ofEpochMilli(MonotonicClock.currentTimeMillis()).plus(backgroundRecoveryInterval)
    ...
    private SortedMap<BitronixTransaction, ClearContextSynchronization> createInFlightTransactionsMap()
        ...
        final Comparator<BitronixTransaction> timestampSortComparator = (t1, t2) -> {
    ...
    public void begin() throws NotSupportedException, SystemException {
        ...
        try {
            ...
            currentTx.setActive(threadContext.getTimeout().toSeconds());
            inFlightTransactions.put(currentTx, clearContextSynchronization);
            if (log.isDebugEnabled()) {
                log.debug("begun new transaction at {}",
                        Instant.ofEpochMilli(currentTx.getResourceManager().getGtrid().extractTimestamp())
                                .atZone(ZoneId.systemDefault()).toLocalDateTime());
            }
    ...
    public void setTransactionTimeout(int seconds) throws SystemException {
        ...
        ThreadContext.getThreadContext().setTimeout(Duration.ofSeconds(seconds));
    }
    ...
    public long getOldestInFlightTransactionTimestamp() {
        try {
            ...
            if (log.isDebugEnabled()) {
                log.debug("oldest in-flight transaction's timestamp: " + Long.MIN_VALUE);
            }
    ...
    private void internalShutdown() {
        ...
        Duration gracefulShutdownInterval = TransactionManagerServices.getConfiguration().getGracefulShutdownInterval();
        int txCount = 0;
        try {
            txCount = inFlightTransactions.size();
            while (!gracefulShutdownInterval.isZero() && txCount > 0) {
                if (log.isDebugEnabled()) {
                    log.debug("still {} in-flight transactions, waiting... ({} left)",
                            txCount, gracefulShutdownInterval);
                }
                try {
                    Thread.sleep(1000);
                ...
                gracefulShutdownInterval = gracefulShutdownInterval.minusSeconds(1L);
                txCount = inFlightTransactions.size();
            }
        } catch (Exception ex) {
        ...
        if (txCount > 0) {
            if (log.isDebugEnabled()) {
                log.debug("still {} in-flight transactions, shutting down anyway", txCount);
                dumpTransactionContexts();
            }
        } else {
            if (log.isDebugEnabled()) {
    ...
    private final class ClearContextSynchronization implements Synchronization {
        ...
        public void afterCompletion(int status) {
            ...
            if (log.isDebugEnabled()) {
                log.debug("removing transaction from in-flight transactions: {}", currentTx);
            }
